...
class TestSessionImmutableHistory:
    ...
    def test_get_history_includes_reasoning_fields(self) -> None:
        """Assistant reasoning_content / thinking_blocks are passed to LLM context."""
        ...
        session.messages.append({
            ...
            "thinking_blocks": [{"type": "thinking", "thinking": "trace"}],
            "timestamp": "2026-01-01T00:00:01",
        })
        history = session.get_history(max_messages=10)
        assert len(history) == 2
        assert history[1]["content"] == "ok"
        assert history[1]["reasoning_content"] == "step a; step b"
        assert history[1]["thinking_blocks"] == [{"type": "thinking", "thinking": "trace"}]
...
class TestNewCommandArchival:
    ...
    async def test_new_does_not_clear_session_when_archive_fails(self, tmp_path: Path) -> None:
        ...
        loop.memory_consolidator.consolidate_messages = _failing_consolidate[method-assign]
    ...
    async def test_new_archives_only_unconsolidated_messages(self, tmp_path: Path) -> None:
        ...
        loop.memory_consolidator.consolidate_messages = _fake_consolidate[method-assign]
    ...
    async def test_new_clears_session_and_responds(self, tmp_path: Path) -> None:
        ...
        loop.memory_consolidator.consolidate_messages = _ok_consolidate[method-assign]
